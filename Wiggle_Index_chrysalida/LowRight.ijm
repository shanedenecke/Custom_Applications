...
dirSav=dirStor+"zCroppedTiffStackLR";
...
tableTitle="Wiggle Index_LowRight";
tableTitle2="["+tableTitle+"]";
if (isOpen("Wiggle Index_LowRight")){
...
for (b=0; b<lis.length; b++){
  ...
  if (endsWith(fileName, ".jpg")) {
    run("Image Sequence...", "open=["+ fileName + "] number=250 sort use");
 	makeRectangle(525, 950, 525, 600);
	run("Crop");
	saveName=getTitle();
    	saveAs("Tiff", dirSav+fs+saveName+"LowRight");
    	close();
...
for (u=0; u<list.length; u++){
	fileName=dirStore+list[u];
		...
		if (endsWith(fileName, ".tif")) {
			...
			selectWindow("Wiggle Index_LowRight");
			saveAs("Text", dirSave+fs+"Wiggle Index LowRight.tsv");
...
//selectWindow("Wiggle Index_LowRight");
//saveAs("Text", parent+fs+hi+"Wiggle Index LowRight.tsv");
run("Quit");
