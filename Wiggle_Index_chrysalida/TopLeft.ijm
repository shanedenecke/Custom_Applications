...
dirSav=dirStor+"zCroppedTiffStackTL";
...
tableTitle="Wiggle Index_TopLeft";
tableTitle2="["+tableTitle+"]";
if (isOpen("Wiggle Index_TopLeft")){
...
for (b=0; b<lis.length; b++){
  ...
  if (endsWith(fileName, ".jpg")) {
    run("Image Sequence...", "open=["+ fileName + "] number=250 sort use");
 	makeRectangle(0, 310, 525, 600);
	run("Crop");
	saveName=getTitle();
    	saveAs("Tiff", dirSav+fs+saveName+"TopLeft");
    	close();
...
for (u=0; u<list.length; u++){
	fileName=dirStore+list[u];
		...
		if (endsWith(fileName, ".tif")) {
			...
			selectWindow("Wiggle Index_TopLeft");
			saveAs("Text", dirSave+fs+"Wiggle Index TopLeft.tsv");
...
//selectWindow("Wiggle Index_TopLeft");
//saveAs("Text", parent+fs+hi+"Wiggle Index TopLeft.tsv");
run("Quit");
