...
dirSav=dirStor+"zCroppedTiffStackTR";
...
tableTitle="Wiggle Index_TopRight";
tableTitle2="["+tableTitle+"]";
if (isOpen("Wiggle Index_TopRight")){
...
for (b=0; b<lis.length; b++){
  ...
  if (endsWith(fileName, ".jpg")) {
    run("Image Sequence...", "open=["+ fileName + "] number=250 sort use");
 	makeRectangle(525, 310, 525, 600);
	run("Crop");
	saveName=getTitle();
    	saveAs("Tiff", dirSav+fs+saveName+"TopRight");
    	close();
...
for (u=0; u<list.length; u++){
	fileName=dirStore+list[u];
		...
		if (endsWith(fileName, ".tif")) {
			...
			selectWindow("Wiggle Index_TopRight");
			saveAs("Text", dirSave+fs+"Wiggle Index TopRight.tsv");
...
//selectWindow("Wiggle Index_TopRight");
//saveAs("Text", parent+fs+hi+"Wiggle Index TopRight.tsv");
run("Quit");
