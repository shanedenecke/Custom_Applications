...
p = add_argument(p, "--indir",help='use domain filter key',default='/mnt/disk/shane/Transporter_ID/Arthropod_ABC_pipeline/ABC_search/HelArm')
...
pfam.table=pfam.scan %>% group_by(name,fam,len) %>% summarize(domains=length(name),.groups="drop_last") %>% data.table()
...
fwrite(filt,'./Filtered_out_ABC.tsv')
