...
cut -f 9 $OUTDIR/$bname/total_ABC_HMM.table | sed 's/\s+//g' | $SCRIPT_DIR/unigene_fa_sub.sh $PROTEOME - > $OUTDIR/$bname/total_ABC_pre_blast.faa
...
$SCRIPT_DIR/fasta_rename.py $PROTEOME $OUTDIR/$bname/total_ABC_dict.csv > $OUTDIR/$bname/total_ABC_marked_proteome.faa
